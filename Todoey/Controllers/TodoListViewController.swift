...
class TodoListViewController: SwipeTableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let item = todoItems?[indexPath.row] {
            ...
            if let hexColor = UIColor(hexString: selectedCategory!.hexColor)?.darken(byPercentage:CGFloat(indexPath.row) / CGFloat(todoItems!.count)) {
    ...
    @IBAction func addButtonPressed(_ sender: UIBarButtonItem) {
        ...
        let action = UIAlertAction(title: "Add Item", style: .default) { (action) in
            ...
            if let currentyCategory = self.selectedCategory {
            do {
                try self.realm.write {
                    let newItem = Item()
                    newItem.title = textField.text!
                    newItem.dateCreated = Date()
                    currentyCategory.items.append(newItem)
                }
            } catch {
    ...
    // external internal parameter and default
    func loadItems() {
